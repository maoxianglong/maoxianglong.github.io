...
3、线程休眠是抱着锁休眠的，我不睡醒谁都别想进来，但是问题却不太一样。
...
三个线程居然同时进入了互斥代码块，同时休眠，同时唤醒，惊呆了。假如t1进入互斥代码块并且睡着了，那么其他两个线程应该乖乖在外面等着，为啥是一起进入还同时休眠了。
...
我将启动线程的对象变为同一个静态对象，并且使用同一个对象调用互斥代码块，看看结果。
